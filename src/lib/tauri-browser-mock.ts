...
const MOCK_PROJECT_SCHEDULED_TASK_RUNS_KEY =
  "devnest.mock.project-scheduled-task-runs";
const MOCK_PROJECT_WORKERS_KEY = "devnest.mock.project-workers";
const MOCK_FRANKENPHP_OCTANE_WORKERS_KEY =
  "devnest.mock.frankenphp-octane-workers";
...
const MOCK_OPTIONAL_TOOL_INSTALL_TASK_KEY =
  "devnest.mock.optional-tool-install-task";
...
const MOCK_PHP_AVAILABLE_EXTENSIONS_KEY =
  "devnest.mock.php-available-extensions";
const MOCK_RUNTIME_CONFIG_OVERRIDES_KEY =
  "devnest.mock.runtime-config-overrides";
const MOCK_LAST_EXPORTED_PROJECT_PROFILE_KEY =
  "devnest.mock.last-exported-project-profile";
...
const MOCK_PROJECT_PERSISTENT_HOSTNAMES_KEY =
  "devnest.mock.project-persistent-hostnames";
const MOCK_PROJECT_PERSISTENT_TUNNELS_KEY =
  "devnest.mock.project-persistent-tunnels";
const MOCK_PERSISTENT_TUNNEL_SETUP_KEY = "devnest.mock.persistent-tunnel-setup";
const MOCK_PERSISTENT_NAMED_TUNNELS_KEY =
  "devnest.mock.persistent-named-tunnels";
...
function buildMockDownloadCacheSummary(
  buckets: DownloadCacheBucket[],
): DownloadCacheSummary {
  return {
    totalSizeBytes: buckets.reduce(
      (total, bucket) => total + bucket.sizeBytes,
      0,
    ),
...
function readMockDownloadCacheSummary(): DownloadCacheSummary {
  ...
  const summary = buildMockDownloadCacheSummary(
    defaultMockDownloadCacheBuckets(),
  );
...
function readMockDatabaseSnapshots(): Record<
  string,
  DatabaseSnapshotSummary[]
> {
  if (typeof window === "undefined") {
...
function writeMockDatabaseSnapshots(
  snapshots: Record<string, DatabaseSnapshotSummary[]>,
) {
  if (typeof window === "undefined") {
    return;
  }
...
function defaultMockDatabaseTimeMachineState(
  enabled: boolean,
): MockDatabaseTimeMachineState {
  return {
    enabled,
...
function readMockDatabaseTimeMachine(): Record<
  string,
  MockDatabaseTimeMachineState
> {
  if (typeof window === "undefined") {
...
function writeMockDatabaseTimeMachine(
  statuses: Record<string, MockDatabaseTimeMachineState>,
) {
  if (typeof window === "undefined") {
    return;
  }
  ...
  window.localStorage.setItem(
    MOCK_DATABASE_TIME_MACHINE_KEY,
...
function mockRedisOptionsDbIndex(
  options?: RedisConnectionOptions | null,
): number {
  const dbIndex = Number(options?.dbIndex ?? 0);
  return Number.isFinite(dbIndex) && dbIndex >= 0 ? dbIndex : 0;
}

function mockRedisStatus(
  options?: RedisConnectionOptions | null,
): RedisManagerStatus {
...
function mockRedisKeyMatches(
  ...
  const typeMatches =
    !typeFilter || typeFilter === "all" || key.keyType === typeFilter;
...
function mockNextScheduledSnapshotAt(
  ...
  if (nextTimestamp <= now) {
    const elapsedIntervals = Math.floor(
      (now - timestamp.getTime()) / intervalMs,
    );
...
function mockDatabaseTimeMachineStatus(
  name: string,
): DatabaseTimeMachineStatus {
  const machineState =
    readMockDatabaseTimeMachine()[name] ??
    defaultMockDatabaseTimeMachineState(false);
  const snapshots = (readMockDatabaseSnapshots()[name] ?? []).sort(
  ...
  return {
    ...
    linkedProjectActionSnapshotsEnabled:
      machineState.linkedProjectActionSnapshotsEnabled,
    latestSnapshotAt,
    nextScheduledSnapshotAt: mockNextScheduledSnapshotAt(
      machineState,
      latestSnapshotAt,
    ),
    lastError: null,
...
function createMockSnapshot(
  ...
  const machineState =
    readMockDatabaseTimeMachine()[name] ??
    defaultMockDatabaseTimeMachineState(true);
  const snapshot: DatabaseSnapshotSummary = {
    ...
    linkedProjectNames:
      options?.linkedProjectNames ?? mockLinkedProjectNamesForDatabase(name),
...
function takeMockProjectLinkedSnapshotIfEnabled(
  project: Project,
  note: string,
) {
...
function writeMockProjectScheduledTasks(tasks: ProjectScheduledTask[]) {
  ...
  window.localStorage.setItem(
    MOCK_PROJECT_SCHEDULED_TASKS_KEY,
...
function readMockProjectScheduledTaskRuns(): ProjectScheduledTaskRun[] {
  ...
  const stored = window.localStorage.getItem(
    MOCK_PROJECT_SCHEDULED_TASK_RUNS_KEY,
  );
...
function writeMockProjectEnvVars(envVars: ProjectEnvVar[]) {
  ...
  window.localStorage.setItem(
    MOCK_PROJECT_ENV_VARS_KEY,
    JSON.stringify(envVars),
  );
}

function buildMockProjectEnvInspection(
  projectId: string,
): ProjectEnvInspection {
  ...
  const diskVars = Array.from(diskMap.values()).sort((left, right) =>
    left.key.localeCompare(right.key),
  );
  const trackedMap = new Map(
    trackedVars.map((item) => [item.envKey, item.envValue]),
  );
  const diskValueMap = new Map(diskVars.map((item) => [item.key, item.value]));
  const allKeys = Array.from(
    new Set([...trackedMap.keys(), ...diskValueMap.keys()]),
  ).sort((left, right) => left.localeCompare(right));
...
function writeMockProjectWorkers(workers: ProjectWorker[]) {
  ...
  window.localStorage.setItem(
    MOCK_PROJECT_WORKERS_KEY,
    JSON.stringify(workers),
  );
}

function readMockFrankenphpOctaneWorkers(): Record<
  string,
  FrankenphpOctaneWorkerSettings
> {
  if (typeof window === "undefined") {
  ...
  const stored = window.localStorage.getItem(
    MOCK_FRANKENPHP_OCTANE_WORKERS_KEY,
  );
  return stored
    ? (JSON.parse(stored) as Record<string, FrankenphpOctaneWorkerSettings>)
    : {};
}

function writeMockFrankenphpOctaneWorkers(
  workers: Record<string, FrankenphpOctaneWorkerSettings>,
) {
  if (typeof window === "undefined") {
    return;
  }
...
function getMockFrankenphpOctaneSettings(
  projectId: string,
): FrankenphpOctaneWorkerSettings {
  ...
  if (stored[projectId]) {
    ...
    return {
      ...stored[projectId],
      mode:
        stored[projectId].mode ??
        (project?.frankenphpMode === "symfony" ||
        project?.frankenphpMode === "custom"
          ? project.frankenphpMode
          : "octane"),
      customWorkerRelativePath:
        stored[projectId].customWorkerRelativePath ?? null,
    };
  }

  const timestamp = new Date().toISOString();
  const project = readMockProjects().find((item) => item.id === projectId);
  const usedWorkerPorts = new Set(
    Object.values(stored).map((worker) => worker.workerPort),
  );
  const usedAdminPorts = new Set(
    Object.values(stored).map((worker) => worker.adminPort),
  );
  const workerPort =
    Array.from({ length: 100 }, (_, index) => 8100 + index).find(
      (port) => !usedWorkerPorts.has(port),
    ) ?? 8100;
  const adminPort =
    Array.from({ length: 100 }, (_, index) => 9100 + index).find(
      (port) => !usedAdminPorts.has(port),
    ) ?? 9100;
...
function buildMockFrankenphpOctaneHealth(
  projectId: string,
): FrankenphpOctaneWorkerHealth {
  const project = getMockProjectOrThrow(projectId);
  ...
  const runtimeLabel = runtime
    ...
    : "FrankenPHP";
  const extensionStates = runtime
    ? mockPhpExtensionsForRuntime(runtime.id, runtimeLabel)
    : [];
  const selectedExtensions = ["redis", "mbstring", "pdo_mysql"].map(
    (extensionName) => {
      const state = extensionStates.find(
  ...
  const uptimeSeconds =
    lastStartedAt && settings.status === "running"
      ? Math.max(
          0,
          Math.floor((Date.now() - new Date(lastStartedAt).getTime()) / 1000),
        )
      : null;
  ...
  return {
    ...
    requestCount:
      settings.status === "running" ? Math.max(1, logs.length * 12) : null,
...
function defaultMockRuntimePackages(): RuntimePackage[] {
  return [
    ...
    },
    {
      ...
      downloadUrl:
        "https://downloads.devnest.invalid/apache-2.4.66-win-x64.zip",
      checksumSha256: "demo-checksum",
      archiveKind: "zip",
    ...
    },
    {
      ...
      downloadUrl:
        "https://downloads.devnest.invalid/frankenphp-1.5.3-win-x64.zip",
      checksumSha256: "demo-checksum",
      archiveKind: "zip",
    ...
    },
    {
      ...
      downloadUrl:
        "https://downloads.devnest.invalid/mariadb-11.8.3-win-x64.zip",
      checksumSha256: "demo-checksum",
      archiveKind: "zip",
...
function mockPhpExtensionsForRuntime(
  runtimeId: string,
  runtimeVersion: string,
): PhpExtensionState[] {
  const timestamp = new Date().toISOString();
  const defaults =
    readMockPhpAvailableExtensions()[runtimeId] ??
    defaultMockPhpAvailableExtensions();
...
function mockPhpFunctionsForRuntime(
  runtimeId: string,
  runtimeVersion: string,
): PhpFunctionState[] {
  const timestamp = new Date().toISOString();
...
function mockRuntimeVersionForType(
...
function mockRuntimePhpFamilyForItem(
...
function findMockServerRuntime(
  project: Project,
  ...
  return (
    runtimes.find(
      (runtime) =>
        runtime.runtimeType === project.serverType && runtime.isActive,
    ) ?? null
  );
}
...
function findMockPhpRuntimeForProject(
  ...
  if (project.serverType === "frankenphp") {
    const resolvedPhpFamily = serverRuntime
      ? mockRuntimePhpFamilyForItem(serverRuntime)
      : null;
    return {
      ...
      matchesVersion: Boolean(
        ...
        resolvedPhpFamily === expectedPhpFamily,
      ),
    };
  }
...
function defaultMockRuntimeConfigValues(
...
function mockRuntimeConfigSchema(
  runtime: RuntimeInventoryItem,
): RuntimeConfigSchema {
  const toggleOptions = [
    { value: "on", label: "On" },
...
function mockRuntimeConfigValues(
  runtime: RuntimeInventoryItem,
): RuntimeConfigValues {
...
function mockPhpExtensionPackagesForRuntime(
  ...
  const phpFamily =
    mockRuntimePhpFamilyForItem(runtime) ??
    mockPhpVersionFamily(runtime.version);
  const requiredThreadSafety =
    runtime.runtimeType === "frankenphp" ? "ts" : null;
...
function writeMockSslAuthorityTrusted(value: boolean) {
  ...
  window.localStorage.setItem(
    MOCK_SSL_AUTHORITY_TRUSTED_KEY,
    value ? "true" : "false",
  );
}
  ...
  return [
    {
      ...
      downloadUrl:
        "https://github.com/axllent/mailpit/releases/download/v1.29.7/mailpit-windows-amd64.zip",
      checksumSha256: null,
      archiveKind: "zip",
    ...
    },
    {
      ...
      checksumSha256:
        "272c1fabc6297302cbb187f4e603d4be4330907b537354a443ee154c4e0ed8a3",
      archiveKind: "binary",
    ...
    },
    {
      ...
      checksumSha256:
        "c2bcaa8ce0f4b942f749c491327dcf126a98169e0bde59013251e179d6f86b8b",
      archiveKind: "zip",
    ...
    },
    {
      ...
      checksumSha256:
        "0c1a713440578cb400d2e76208feb24f1b339426b075a21f73b6b2132692515d",
      archiveKind: "zip",
...
function writeMockOptionalToolPackages(packages: OptionalToolPackage[]) {
  ...
  window.localStorage.setItem(
    MOCK_OPTIONAL_TOOL_PACKAGES_KEY,
    JSON.stringify(packages),
  );
}
...
function readMockOptionalToolInstallTask(): OptionalToolInstallTask | null {
  ...
  const stored = window.localStorage.getItem(
    MOCK_OPTIONAL_TOOL_INSTALL_TASK_KEY,
  );
  return stored ? (JSON.parse(stored) as OptionalToolInstallTask) : null;
}

function writeMockOptionalToolInstallTask(
  task: OptionalToolInstallTask | null,
) {
  if (typeof window === "undefined") {
    return;
  }
...
function writeMockPhpExtensionOverrides(value: MockPhpExtensionOverrides) {
  ...
  window.localStorage.setItem(
    MOCK_PHP_EXTENSION_OVERRIDES_KEY,
...
function writeMockPhpFunctionOverrides(value: MockPhpFunctionOverrides) {
  ...
  window.localStorage.setItem(
    MOCK_PHP_FUNCTION_OVERRIDES_KEY,
    JSON.stringify(value),
  );
}
...
function writeMockPhpAvailableExtensions(value: MockPhpAvailableExtensions) {
  ...
  window.localStorage.setItem(
    MOCK_PHP_AVAILABLE_EXTENSIONS_KEY,
    JSON.stringify(value),
  );
}
...
function writeMockRuntimeConfigOverrides(value: MockRuntimeConfigOverrides) {
  ...
  window.localStorage.setItem(
    MOCK_RUNTIME_CONFIG_OVERRIDES_KEY,
    JSON.stringify(value),
  );
}
...
function readMockLastExportedProjectProfile(): Record<string, unknown> | null {
  ...
  const stored = window.localStorage.getItem(
    MOCK_LAST_EXPORTED_PROJECT_PROFILE_KEY,
  );
...
function readMockLastExportedTeamProjectProfile(): Record<
  string,
  unknown
> | null {
  if (typeof window === "undefined") {
...
function writeMockLastExportedTeamProjectProfile(
...
function buildMockFrankenphpProductionExportPreview(
  projectId: string,
): FrankenphpProductionExportPreview {
  const project = getMockProjectOrThrow(projectId);
  if (project.serverType !== "frankenphp") {
  ...
  const settings =
    project.frankenphpMode === "classic"
      ? null
      : getMockFrankenphpOctaneSettings(projectId);
  const slug = mockProductionExportSlug(project.domain);
  const documentRoot =
    project.documentRoot === "."
      ? `/var/www/${slug}`
      : `/var/www/${slug}/${project.documentRoot}`;
  const warnings = [
...
function readMockProjectTunnels(): Record<string, ProjectTunnelState> {
  ...
  return stored
    ? (JSON.parse(stored) as Record<string, ProjectTunnelState>)
    : {};
}
...
function readMockProjectMobilePreviews(): Record<
  string,
  ProjectMobilePreviewState
> {
  if (typeof window === "undefined") {
    return {};
  }
  ...
  return stored
    ? (JSON.parse(stored) as Record<string, ProjectMobilePreviewState>)
    : {};
}

function writeMockProjectMobilePreviews(
  value: Record<string, ProjectMobilePreviewState>,
) {
  if (typeof window === "undefined") {
    return;
  }
  ...
  window.localStorage.setItem(
    MOCK_PROJECT_MOBILE_PREVIEWS_KEY,
    JSON.stringify(value),
  );
}

function readMockProjectPersistentHostnames(): Record<
  string,
  ProjectPersistentHostname
> {
  if (typeof window === "undefined") {
  ...
  return stored
    ? (JSON.parse(stored) as Record<string, ProjectPersistentHostname>)
    : {};
}
...
function readMockProjectPersistentTunnels(): Record<
  string,
  ProjectPersistentTunnelState
> {
  if (typeof window === "undefined") {
  ...
  return stored
    ? (JSON.parse(stored) as Record<string, ProjectPersistentTunnelState>)
    : {};
}
...
function writeMockPersistentTunnelSetup(value: PersistentTunnelSetupStatus) {
  ...
  window.localStorage.setItem(
    MOCK_PERSISTENT_TUNNEL_SETUP_KEY,
    JSON.stringify(value),
  );
}
...
function writeMockPersistentNamedTunnels(
  value: PersistentTunnelNamedTunnelSummary[],
) {
  if (typeof window === "undefined") {
    return;
  }
  ...
  window.localStorage.setItem(
    MOCK_PERSISTENT_NAMED_TUNNELS_KEY,
    JSON.stringify(value),
  );
}
...
function getMockScheduledTaskOrThrow(taskId: string): ProjectScheduledTask {
  const task = readMockProjectScheduledTasks().find(
    (item) => item.id === taskId,
  );
  if (!task) {
    throw {
...
function upsertMockScheduledTask(
...
function mockNextScheduledTaskRunAt(
  ...
  switch (task.simpleScheduleKind) {
    ...
    case "everyHours":
      baseDate.setTime(
        baseDate.getTime() + (task.intervalSeconds ?? 300) * 1000,
      );
      return baseDate.toISOString();
    case "daily": {
      const [hours, minutes] = (task.dailyTime ?? "08:00")
        .split(":")
        .map(Number);
...
function inferFrameworkFromPath(path: string): ScanResult {
  ...
  if (normalized.includes("laravel")) {
    return {
      ...
      serverReason:
        "Browser mock uses folder-name heuristics and defaults Laravel to Nginx.",
      recommendedPhpVersion: "8.2",
      suggestedDomain: `${name}.test`,
      documentRoot: "public",
      documentRootReason:
        "Browser mock found a Laravel-like path and assumed public/ as the web root.",
  ...
  if (normalized.includes("symfony")) {
    return {
      ...
      recommendedServer: "nginx",
      serverReason:
        "Browser mock uses folder-name heuristics and defaults Symfony to Nginx.",
      recommendedPhpVersion: "8.2",
      suggestedDomain: `${name}.test`,
      documentRoot: "public",
      documentRootReason:
        "Browser mock found a Symfony-like path and assumed public/ as the web root.",
  ...
  if (normalized.includes("wordpress") || normalized.includes("wp")) {
    return {
      ...
      serverReason:
        "Browser mock uses folder-name heuristics and assumes WordPress prefers Apache.",
      suggestedDomain: `${name}.test`,
      documentRoot: ".",
      documentRootReason:
        "Browser mock assumed WordPress serves from the project root.",
  ...
  return {
    ...
    detectedFiles: normalized.includes("public")
      ? ["public/index.php"]
      : ["index.php"],
...
function validateMockProjectInput(input: CreateProjectInput) {
  ...
  if (!pathResult.success) {
    throw {
      ...
      message:
        pathResult.error.issues[0]?.message ?? "Project path is required.",
    } satisfies AppError;
  }
  ...
  if (!nameResult.success) {
    throw {
      ...
      message:
        nameResult.error.issues[0]?.message ?? "Project name is invalid.",
    } satisfies AppError;
  }
  ...
  if (!domainResult.success) {
    throw {
      ...
      message:
        domainResult.error.issues[0]?.message ?? "Project domain is invalid.",
    } satisfies AppError;
  }
  ...
  if (!documentRootResult.success) {
    throw {
      ...
      message:
        documentRootResult.error.issues[0]?.message ??
        "Document root is invalid.",
    } satisfies AppError;
  }
...
function createMockRecipeProject(
  ...
  if (!pathResult.success) {
    throw {
      ...
      message:
        pathResult.error.issues[0]?.message ??
  ...
  const rawName =
    segments.length > 0 ? segments[segments.length - 1] : "project";
  const name = rawName
    .split(/[-_\s]+/)
    .filter(Boolean)
...
function mockDiagnostics(projectId: string): DiagnosticItem[] {
  ...
  const runtimeService = services.find(
    (service) => service.name === project.serverType,
  );
  ...
  if (project.framework === "laravel" && project.documentRoot !== "public") {
    items.push({
      ...
      suggestion:
        "Change the document root to `public`, then regenerate the managed config.",
      createdAt: timestamp,
    });
  }
  ...
  if (project.serverType === "apache" && project.framework === "laravel") {
    items.push({
      ...
      suggestion:
        "Run the desktop app to verify `mod_rewrite` against the real Apache runtime.",
      createdAt: timestamp,
    });
  }
  ...
  if (project.framework === "laravel" && project.phpVersion === "8.1") {
    items.push({
      ...
      message:
        "Laravel usually needs extensions such as intl and mbstring before the site can run cleanly.",
      suggestion:
        "Verify the selected PHP runtime and enable the required extensions before starting the site.",
      createdAt: timestamp,
    });
  }
  ...
  if (project.databaseName && mysqlService?.status === "error") {
    items.push({
      ...
      suggestion:
        mysqlService.lastError ??
        "Inspect the MySQL log and retry after fixing the startup issue.",
      createdAt: timestamp,
    });
  }
  ...
  if (items.length === 0) {
    items.push({
      ...
      message:
        "The current project profile looks ready for a standard local run.",
...
function buildMockFrankenphpOctanePreflight(
  projectId: string,
): FrankenphpOctanePreflight {
  const project = getMockProjectOrThrow(projectId);
  const settings = getMockFrankenphpOctaneSettings(projectId);
  const mode =
    project.frankenphpMode === "symfony" || project.frankenphpMode === "custom"
      ? project.frankenphpMode
      : "octane";
  const runtime = readMockRuntimes().find(
    (item) => item.runtimeType === "frankenphp" && item.isActive,
  ...
  const workerConflict = readMockServices().find(
    (service) =>
      service.status === "running" && service.port === settings.workerPort,
  );
  const adminConflict = readMockServices().find(
    (service) =>
      service.status === "running" && service.port === settings.adminPort,
  );
  const checks: FrankenphpOctanePreflight["checks"] = [
    ...
    },
    {
      ...
      suggestion:
        project.serverType === "frankenphp"
          ? null
          : "Change the project server to FrankenPHP.",
      ...
      level:
        mode === "octane"
          ? project.name.toLowerCase().includes("octane")
            ...
            : "error"
          : mode === "symfony"
            ? project.name.toLowerCase().includes("runtime") ||
              project.name.toLowerCase().includes("symfony")
              ? "ok"
              : "error"
    ...
    },
    {
      ...
      suggestion: runtime
        ? null
        : "Link or activate FrankenPHP in Settings before starting Octane.",
  ...
  return {
    ...
    installCommands: checks.some(
      (item) => item.title === "Laravel Octane package" && item.blocking,
    )
...
function buildMockConfigPreview(project: Project) {
  ...
  if (
    (project.framework === "laravel" || project.framework === "symfony") &&
    project.documentRoot !== "public"
  ) {
    throw {
...
function getMockResponse<T>(command: string): T {
  switch (command) {
    ...
    case "refresh_runtime_inventory":
...
export function getMockResponseWithArgs<T>(
  command: string,
  ...
  switch (command) {
    ...
    case "create_project": {
      ...
      if (created.databaseName) {
        const nextDatabases = Array.from(
          new Set([...readMockDatabases(), created.databaseName]),
        ).sort((left, right) => left.localeCompare(right));
    ...
    case "create_laravel_recipe": {
      ...
      return createMockRecipeProject({
        ...
        serverType: (input?.serverType ??
          "apache") as CreateProjectInput["serverType"],
    ...
    case "clone_git_recipe": {
      ...
      const repositoryCheck = gitRepositoryUrlSchema.safeParse(
        String(input?.repositoryUrl ?? ""),
      );
      if (!repositoryCheck.success) {
        throw {
          code: "INVALID_REPOSITORY_URL",
          message:
            repositoryCheck.error.issues[0]?.message ??
      ...
      return createMockRecipeProject({
        ...
        phpVersion: String(input?.phpVersion ?? "8.4"),
        serverType: (input?.serverType ??
          inferred.recommendedServer) as CreateProjectInput["serverType"],
        sslEnabled: Boolean(input?.sslEnabled),
    ...
    case "update_project": {
      ...
      validateMockProjectInput({
        ...
        databaseName:
          patch.databaseName === undefined
            ? (project.databaseName ?? null)
            : patch.databaseName,
        databasePort:
          patch.databasePort === undefined
            ? (project.databasePort ?? null)
            : patch.databasePort,
        frankenphpMode:
          patch.frankenphpMode ?? project.frankenphpMode ?? "classic",
      });
      ...
      if ((project.databaseName ?? null) !== nextDatabaseName) {
        takeMockProjectLinkedSnapshotIfEnabled(
          project,
          `before relinking project ${project.name}`,
        );
      }

      writeMockProjects(
        existing.map((item) => (item.id === projectId ? updated : item)),
      );
      return updated as T;
    }
              ...
              ? {
                  ...
                  workerPolicy:
                    project.frankenphpMode === "classic"
                      ? null
                      : {
                          workers: getMockFrankenphpOctaneSettings(project.id)
                            .workers,
                          maxRequests: getMockFrankenphpOctaneSettings(
                            project.id,
                          ).maxRequests,
                          preferredWorkerPort: getMockFrankenphpOctaneSettings(
                            project.id,
                          ).workerPort,
                          preferredAdminPort: getMockFrankenphpOctaneSettings(
                            project.id,
                          ).adminPort,
                          customWorkerRelativePath:
                            getMockFrankenphpOctaneSettings(project.id)
                              .customWorkerRelativePath,
                        },
                  runtimeRequirements: {
                  ...
                  localDiagnostics:
                    project.frankenphpMode === "classic"
                      ? null
                      : {
                          currentWorkerPort: getMockFrankenphpOctaneSettings(
                            project.id,
                          ).workerPort,
                          currentAdminPort: getMockFrankenphpOctaneSettings(
                            project.id,
                          ).adminPort,
                          workerLogPath: getMockFrankenphpOctaneSettings(
                            project.id,
                          ).logPath,
                        },
                }
            ...
            ? [
                {
                  ...
                  suggestion:
                    "Share secret values through your team's secret manager.",
                },
              ]
            : [],
      } as T;
    ...
    case "import_project_profile": {
      ...
      if (!profile) {
        throw {
          ...
          message:
            "No preview project profile is available to import yet. Export one first in preview mode.",
        } satisfies AppError;
      }
      ...
      const existing = readMockProjects();
      if (
        existing.some((project) => project.domain === projectProfile.domain)
      ) {
        throw {
          code: "DOMAIN_ALREADY_EXISTS",
          message: "A project with this domain already exists.",
          ...
          ? [
              {
                ...
                suggestion:
                  "Check the Runtime tab after import if ports conflict locally.",
              },
            ]
          : [];
    ...
    case "preview_frankenphp_production_export": {
      return buildMockFrankenphpProductionExportPreview(
        String(args?.projectId ?? ""),
      ) as T;
    }
    case "write_frankenphp_production_export": {
      const preview = buildMockFrankenphpProductionExportPreview(
        String(args?.projectId ?? ""),
      );
      return {
        success: true,
        ...
        files: preview.files.map(
          (file) =>
            `${MOCK_APP_DATA_ROOT}/production-exports/${preview.slug}/${file.relativePath}`,
        ),
    ...
    case "create_project_env_var": {
      const input = args?.input as
        { projectId: string; envKey: string; envValue: string } | undefined;
      if (!input) {
        throw {
          code: "INVALID_INPUT",
      ...
      const envKey = String(input.envKey ?? "")
        .trim()
        .toUpperCase();
      ...
      if (
        envVars.some(
          (item) =>
            item.projectId === input.projectId && item.envKey === envKey,
        )
      ) {
        throw {
    ...
    case "update_project_env_var": {
      ...
      const current = envVars.find(
        (item) =>
          item.projectId === input.projectId && item.id === input.envVarId,
      );
      if (!current) {
        throw {
      ...
      writeMockProjectEnvVars(
        envVars.map((item) => (item.id === updated.id ? updated : item)),
      );
      return updated as T;
    }
    case "delete_project_env_var": {
      ...
      const envVars = readMockProjectEnvVars();
      const current = envVars.find(
        (item) => item.projectId === projectId && item.id === envVarId,
      );
      if (!current) {
        throw {
          code: "PROJECT_ENV_VAR_NOT_FOUND",
    ...
    case "create_database": {
      ...
      writeMockDatabases(
        [...databases, name].sort((left, right) => left.localeCompare(right)),
      );
    ...
    case "rollback_database_snapshot": {
      ...
      const safetySnapshot = takeMockPreActionSnapshotIfEnabled(
        name,
        "before rollback",
      );
      return {
    ...
    case "drop_database": {
      ...
      const name = String(args?.name ?? "").trim();
      const dependentProjects = readMockProjects().filter(
        (project) => project.databaseName === name,
      );
    ...
    case "delete_project": {
      ...
      takeMockProjectLinkedSnapshotIfEnabled(
        project,
        `before deleting project ${project.name}`,
      );
      writeMockProjects(
        readMockProjects().filter((item) => item.id !== projectId),
      );
      writeMockProjectEnvVars(
      ...
      const removedRunIds = readMockProjectScheduledTaskRuns()
        .filter(
          (item) =>
            ...
            removedTaskIds.includes(item.taskId),
        )
        .map((item) => item.id);
      writeMockProjectScheduledTasks(
        readMockProjectScheduledTasks().filter(
    ...
    case "create_project_scheduled_task": {
      ...
      const parsedCommand = input.commandLine
        ? parseMockWorkerCommandLine(input.commandLine)
        : null;
      ...
      const created: ProjectScheduledTask = {
        ...
        scheduleMode: input.scheduleMode,
        simpleScheduleKind:
          input.scheduleMode === "simple"
            ? (input.simpleScheduleKind ?? null)
            : null,
        scheduleExpression,
        intervalSeconds: input.intervalSeconds ?? null,
        dailyTime: input.dailyTime ?? null,
    ...
    case "update_project_scheduled_task": {
      ...
      const dailyTime =
        patch.dailyTime === undefined
          ? (current.dailyTime ?? null)
          : patch.dailyTime;
      const weeklyDay =
        patch.weeklyDay === undefined
          ? (current.weeklyDay ?? null)
          : patch.weeklyDay;
      const parsedCommand =
      ...
      const nextTask: ProjectScheduledTask = {
        ...
        scheduleMode,
        simpleScheduleKind:
          scheduleMode === "simple" ? (simpleScheduleKind ?? null) : null,
          ...
          (patch.taskType ?? current.taskType) === "command"
            ? patch.workingDirectory === undefined
              ? (current.workingDirectory ??
                getMockProjectOrThrow(current.projectId).path)
    ...
    case "delete_project_scheduled_task": {
      ...
      const removedRunIds = readMockProjectScheduledTaskRuns()
        ...
        .map((run) => run.id);
      writeMockProjectScheduledTasks(
        readMockProjectScheduledTasks().filter((task) => task.id !== taskId),
      );
      writeMockProjectScheduledTaskRuns(
        readMockProjectScheduledTaskRuns().filter(
    ...
    case "enable_project_scheduled_task": {
      ...
      return upsertMockScheduledTask({
        ...
        nextRunAt: mockNextScheduledTaskRunAt(
          { ...task, enabled: true },
          timestamp,
        ),
        updatedAt: timestamp,
    ...
    case "run_project_scheduled_task_now": {
      ...
      const run: ProjectScheduledTaskRun = {
        ...
        errorMessage: failed
          ? `${task.name} failed in browser mock mode.`
          : null,
      ...
      const updatedTask = upsertMockScheduledTask({
        ...
        nextRunAt: task.enabled
          ? mockNextScheduledTaskRunAt(task, timestamp)
          : null,
        lastRunAt: timestamp,
    ...
    case "read_project_scheduled_task_run_logs": {
      ...
      const run = readMockProjectScheduledTaskRuns().find(
        (item) => item.id === runId,
      );
      if (!run) {
        throw {
      ...
      return {
        ...
        lines: selected.map((text, index) => ({
          ...
          text,
          severity: /error|fatal/i.test(text)
            ? "error"
            : /warn/i.test(text)
              ? "warning"
              : "info",
          lineNumber:
            Math.max((logs[logKey]?.length ?? 0) - selected.length, 0) +
            index +
            1,
        })),
    ...
    case "create_project_worker": {
      ...
      const project = getMockProjectOrThrow(input.projectId);
      const { command, args: parsedArgs } = parseMockWorkerCommandLine(
        input.commandLine,
      );
      if (!command) {
        throw {
    ...
    case "delete_project_worker": {
      const workerId = String(args?.workerId ?? "");
      writeMockProjectWorkers(
        readMockProjectWorkers().filter((worker) => worker.id !== workerId),
      );
    ...
    case "start_project_worker": {
      ...
      appendMockWorkerLog(
        worker.id,
        `[info] ${worker.name} started from ${worker.workingDirectory}`,
      );
      return upsertMockWorker({
        ...worker,
        status: "running",
    ...
    case "install_optional_tool_package": {
      ...
      const targetPackage = readMockOptionalToolPackages().find(
      ...
      const tools = readMockOptionalTools()
        .map((item) =>
          item.toolType === targetPackage.toolType
    ...
    case "remove_optional_tool": {
      ...
      if (
        target.toolType === "mailpit" &&
        getMockServiceOrThrow("mailpit").status === "running"
      ) {
        throw {
    ...
    case "start_project_tunnel": {
      ...
      const tunnel: ProjectTunnelState = {
        ...
        updatedAt: timestamp,
        details:
          "Mock tunnel is active through the optional cloudflared integration.",
      };
    ...
    case "create_persistent_named_tunnel": {
      ...
      if (
        existingTunnels.some(
          (item) =>
            normalizeMockTunnelNameKey(item.tunnelName) ===
            normalizeMockTunnelNameKey(name),
        )
      ) {
        throw {
      ...
      const next = {
        ...
        authCertPath:
          ...
          `${MOCK_APP_DATA_ROOT}/persistent-tunnels/cloudflared/cert.pem`,
        credentialsPath: `${MOCK_APP_DATA_ROOT}/persistent-tunnels/cloudflared/credentials/${tunnelId}.json`,
        tunnelId,
        tunnelName: name,
    ...
    case "select_persistent_named_tunnel": {
      ...
      writeMockPersistentNamedTunnels(
        tunnels.map((item) => ({
          ...item,
          selected: item.tunnelId === tunnelId,
        })),
      );
      return next as T;
    }
    case "delete_persistent_named_tunnel": {
      ...
      const remainingTunnels = tunnels.filter(
    ...
    case "update_persistent_tunnel_setup": {
      ...
      const zone =
        String(input.defaultHostnameZone ?? "")
          .trim()
          .toLowerCase() || null;
      const current = readMockPersistentTunnelSetup();
      const next = {
    ...
    case "apply_project_persistent_hostname": {
      ...
      const project = getMockProjectOrThrow(projectId);
      const setup = readMockPersistentTunnelSetup();
      const rawHostname = String(input.hostname ?? "")
        .trim()
        .toLowerCase();
      ...
      const duplicate = Object.values(hostnames).find(
        (entry) =>
          entry.hostname === resolvedHostname && entry.projectId !== projectId,
      );
      if (duplicate) {
        throw {
          code: "PERSISTENT_HOSTNAME_EXISTS",
          message:
            "That persistent hostname is already assigned to another project.",
        } satisfies AppError;
      }
    ...
    case "unpublish_project_persistent_tunnel": {
      ...
      return {
        ...
        binaryPath: `${MOCK_APP_DATA_ROOT}/optional-tools/cloudflared/cloudflared.exe`,
        tunnelId:
          readMockPersistentTunnelSetup().tunnelId ?? "mock-devnest-tunnel",
    ...
    case "open_project_persistent_tunnel_url": {
      ...
      if (!tunnel?.publicUrl) {
        throw {
          ...
          message:
            "Start the persistent tunnel first so DevNest has a stable public URL to open.",
        } satisfies AppError;
      }
      if (typeof window !== "undefined") {
        window.open(tunnel.publicUrl, "_blank", "noopener,noreferrer");
    ...
    case "open_service_dashboard": {
      ...
      if (typeof window !== "undefined") {
        window.open(
          `http://127.0.0.1:${service.port ?? 8025}`,
          "_blank",
          "noopener,noreferrer",
        );
      }
    ...
    case "recover_web_port_from_wsl": {
      const service = getMockServiceOrThrow(String(args?.name ?? ""));
      if (
        !(["apache", "nginx", "frankenphp"] as ServiceState["name"][]).includes(
          service.name,
        )
      ) {
        throw {
          code: "WSL_RECOVERY_UNSUPPORTED_SERVICE",
          message:
            "WSL port recovery is only available for managed web servers.",
        } satisfies AppError;
      }
      const recovered: ServiceState = {
        ...service,
        pid: Math.floor(Math.random() * 40000) + 1000,
        status: "running",
        lastError: null,
      ...
      return upsertMockService(recovered) as T;
    }
    case "stop_service": {
      const service = getMockServiceOrThrow(String(args?.name ?? ""));
        ...
        lines: selected.map((text, index) => ({
          ...
          severity: /error|fatal/i.test(text)
            ...
            : /warn/i.test(text)
              ...
              : "info",
          lineNumber:
            Math.max((logs[name]?.length ?? 0) - selected.length, 0) +
            index +
            1,
        })),
        content: selected.join("\n"),
    ...
    case "get_redis_manager_status": {
      return mockRedisStatus(
        (args?.options ?? null) as RedisConnectionOptions | null,
      ) as T;
    }
    case "list_redis_keys": {
      ...
      return {
        ...
        pattern,
        typeFilter:
          input.typeFilter === "all" ? null : (input.typeFilter ?? null),
    ...
    case "update_project_frankenphp_worker_settings": {
      ...
      const input = (args?.input ??
        {}) as UpdateFrankenphpOctaneWorkerSettingsInput;
    ...
    case "reload_project_frankenphp_worker": {
      ...
      logs[logKey] = [
        ...(logs[logKey] ?? []),
        `[${timestamp}] FrankenPHP worker reload requested.`,
      ];
      writeMockServiceLogs(logs);
      return updateMockFrankenphpOctaneSettings(projectId, {
        lastError: null,
    ...
    case "check_port": {
      ...
      return {
        ...
        conflictSource: conflict ? "other" : null,
    ...
    case "run_action_preflight": {
      const action = String(
        args?.action ?? "provisionProject",
      ) as ActionPreflightReport["action"];
      const projectId = args?.projectId ? String(args.projectId) : null;
      const project = projectId ? getMockProjectOrThrow(projectId) : null;
      const services = readMockServices();
      const runtimes = readMockRuntimes();
      const hosts = readMockHosts();
      const hostname = projectId
        ? readMockProjectPersistentHostnames()[projectId]
        : null;
      const setup = readMockPersistentTunnelSetup();
      ...
      if (action === "restoreAppMetadata") {
        ...
        checks.push({
          ...
          suggestion:
            runningServices > 0
              ? "Use Services or Reliability to stop the workspace first."
              : null,
        });
      }
      ...
      if (project) {
        ...
        const phpBinding = findMockPhpRuntimeForProject(
          project,
          runtimes,
          serverRuntime,
        );
        ...
        checks.push({
          ...
          suggestion: phpBinding.matchesVersion
            ? null
            : phpBinding.embedded
      ...
      return {
        ...
        runtime: {
          ...
          php: {
            ...
            version:
              phpBinding.resolvedPhpFamily ??
              phpBinding.runtime?.version ??
              null,
        ...
        persistentHostname:
          readMockProjectPersistentHostnames()[projectId] ?? null,
        persistentTunnel: readMockProjectPersistentTunnels()[projectId] ?? null,
        persistentHealth:
          readMockProjectPersistentHostnames()[projectId] ||
          readMockProjectPersistentTunnels()[projectId]
            ? ({
                projectId,
                hostname:
                  readMockProjectPersistentHostnames()[projectId]?.hostname ??
                  null,
                overallStatus:
                  readMockProjectPersistentTunnels()[projectId]?.status ??
                  "starting",
                updatedAt: timestamp,
                checks: [
                  {
                    code: "setup",
                    label: "Named Tunnel Setup",
                    status: readMockPersistentTunnelSetup().ready
                      ? "running"
                      : "error",
                  ...
                  },
                  {
                    code: "hostname",
                    label: "Reserved Hostname",
                    status: readMockProjectPersistentHostnames()[projectId]
                      ? "running"
                      : "error",
    ...
    case "run_repair_workflow": {
      ...
      ) as RepairExecutionResult["workflow"];
      const project = getMockProjectOrThrow(projectId);
      if (workflow === "project") {
        writeMockProjects(
    ...
    case "install_php_extension": {
      ...
      const currentExtensions =
        availableExtensions[runtime.id] ?? defaultMockPhpAvailableExtensions();
      const nextExtensionName = `custom_${currentExtensions.length + 1}`;
      availableExtensions[runtime.id] = Array.from(
        new Set([...currentExtensions, nextExtensionName]),
      ).sort();
    ...
    case "remove_php_extension": {
      const runtimeId = String(args?.runtimeId ?? "");
      const extensionName = String(args?.extensionName ?? "")
        .trim()
        .toLowerCase();
      const runtime = readMockRuntimes().find((item) => item.id === runtimeId);
      if (!isMockPhpToolsRuntime(runtime)) {
    ...
    case "set_php_extension_enabled": {
      ...
      const states = mockPhpExtensionsForRuntime(
        runtime.id,
        mockPhpToolsRuntimeLabel(runtime),
      );
    ...
    case "verify_runtime_path": {
      const runtimeType = String(
        args?.runtimeType ?? "php",
      ) as RuntimeInventoryItem["runtimeType"];
      const path = String(args?.path ?? "");
      if (!path.trim()) {
        throw {
      ...
      return {
        ...
        phpFamily: mockRuntimePhpFamilyForType(
          runtimeType,
          mockRuntimeVersionForType(runtimeType),
        ),
        path,
        isActive: false,
        source: "external",
    ...
    case "install_runtime_package": {
      ...
      const targetPackage = readMockRuntimePackages().find(
        (item) => item.id === packageId,
      );
      if (!targetPackage) {
        throw {
    ...
    case "import_runtime_path": {
      ...
      const version = mockRuntimeVersionForType(runtimeType);
      const managedPath = mockManagedRuntimeBinaryPath(
        runtimeType,
        version,
        "managed",
      );
    ...
    case "remove_runtime_reference": {
      ...
      const dependentProjects = readMockProjects().filter((project) => {
        if (target.runtimeType === "php") {
          return (
            project.serverType !== "frankenphp" &&
            mockPhpVersionFamily(project.phpVersion) ===
              mockRuntimePhpFamilyForItem(target)
          );
        }
