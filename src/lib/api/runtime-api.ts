...
export const runtimeApi = {
  ...
  refresh: () =>
    tauriInvoke<RuntimeInventoryItem[]>("refresh_runtime_inventory"),
  listPackages: () => tauriInvoke<RuntimePackage[]>("list_runtime_packages"),
  getConfigSchema: (runtimeId: string) =>
    tauriInvoke<RuntimeConfigSchema>("get_runtime_config_schema", {
      runtimeId,
    }),
  getConfigValues: (runtimeId: string) =>
    tauriInvoke<RuntimeConfigValues>("get_runtime_config_values", {
      runtimeId,
    }),
  updateConfig: (runtimeId: string, patch: Record<string, string>) =>
    tauriInvoke<RuntimeConfigValues>("update_runtime_config", {
      runtimeId,
      patch,
    }),
  ...
  verify: (runtimeType: string, path: string) =>
    tauriInvoke<RuntimeInventoryItem>("verify_runtime_path", {
      runtimeType,
      path,
    }),
  link: (runtimeType: string, path: string, setActive = true) =>
    tauriInvoke<RuntimeInventoryItem>("link_runtime_path", {
      runtimeType,
      path,
      setActive,
    }),
  import: (runtimeType: string, path: string, setActive = true) =>
    tauriInvoke<RuntimeInventoryItem>("import_runtime_path", {
      runtimeType,
      path,
      setActive,
    }),
  installPackage: (packageId: string, setActive = true) =>
    tauriInvoke<RuntimeInventoryItem>("install_runtime_package", {
      packageId,
      setActive,
    }),
  getInstallTask: () =>
    tauriInvoke<RuntimeInstallTask | null>("get_runtime_install_task"),
  ...
  listPhpExtensionPackages: (runtimeId: string) =>
    tauriInvoke<PhpExtensionPackage[]>("list_php_extension_packages", {
      runtimeId,
    }),
  ...
  installPhpExtension: (runtimeId: string) =>
    tauriInvoke<PhpExtensionInstallResult | null>("install_php_extension", {
      runtimeId,
    }),
